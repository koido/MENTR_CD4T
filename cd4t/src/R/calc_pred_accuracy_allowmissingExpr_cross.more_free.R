...
#--------------
#
# Calculate predictive accuracy (Use truth set from phen.gz)
#
#--------------
...
parser$add_argument("--pred_file", required = T,
                    help = "Path for input file (prediction). Header is required. [*_type.test.expr_pred.txt.gz]")
parser$add_argument("--truth_phen", required = T,
                    help = "Path for input file (evaluation). Header is NOT required. [*.phen.gz]")
...
pred <- read_delim(file = args$pred_file, delim = "\t", col_types = args$col_types)
colnames(pred)[colnames(pred) == args$header_prediction] <- "pred"

# read truth file
## col1: clusterID
## col2: expr (expressed:1 or not:0)
truth <- read_delim(file = args$truth_phen, delim = "\t", col_types = "ci", col_names = F)
colnames(truth) <- c("clusterID", "expr")

# merge truth and prediction
# remove expr clumn from pred
pred <- pred %>% dplyr::select(-expr)
# merge
pred <- pred %>%
  inner_join(x = ., y = truth %>% select(clusterID, expr), by = "clusterID")
...
# save
out_file = file.path(args$out_dir_cmn, "each", "cross", args$out_name)
dir.create(file.path(args$out_dir_cmn, "each", "cross"), showWarnings = F, recursive = T)
