...
parser$add_argument("--truth_file", required = T,
                    help = "Path for input file (evaluation). Header is required. [*_type.test.expr_pred.txt.gz]")
...
# read truth file
truth <- read_delim(file = args$truth_file, delim = "\t", col_types = args$col_types)
if(args$header_expression == "auto"){
  colnames(truth)[colnames(truth) == "expr"] <- "expr"
}else{
  colnames(truth)[colnames(truth) == args$header_expression] <- "expr"
}
